...
def do_experiments(start, end, step_num):
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        plt.plot(xx, slope * xx + intercept, color='green', linewidth=2)

        # Implement: Calculate and store logistic loss
        y_pred_proba = model.predict_proba(X)[:, 1]
        y_pred_proba = np.clip(y_pred_proba, 1e-15, 1 - 1e-15)
        log_loss_value = -np.mean(y * np.log(y_pred_proba) + (1 - y) * np.log(1 - y_pred_proba))
        loss_list.append(log_loss_value)
    ...
    plt.plot(xx, slope * xx + intercept, color='green', linewidth=2, label="Decision Boundary")
    plt.tight_layout()
